...
window.exportBriefPDF = function exportBriefPDF() {
    ...
    try {
        ...
        doc.setFontSize(18);
        doc.text('AIRLINE PILOT BRIEFING', 105, 18, { align: 'center' });
        doc.setFontSize(11);
        doc.setFont('helvetica', 'normal');
        doc.text('Flight: ' + (document.getElementById('pp-callsign')?.value || '________'), 20, 30);
        doc.text('Date: ' + new Date().toLocaleDateString(), 150, 30);
        doc.text('Departure: ' + (document.getElementById('pp-dep')?.value || '____'), 20, 38);
        doc.text('Arrival: ' + (document.getElementById('pp-arr')?.value || '____'), 150, 38);
        doc.line(20, 42, 190, 42);

        // Section: Notes (from scratchpad)
        doc.setFontSize(13);
        doc.setFont('helvetica', 'bold');
        doc.text('Briefing Notes', 20, 50);
        doc.setFontSize(11);
        doc.setFont('helvetica', 'normal');
        const notes = (document.getElementById('pp-brief')?.value || '').trim();
        const lines = doc.splitTextToSize(notes || 'No notes provided.', 170);
        ...
        doc.text(doc.splitTextToSize(route || 'N/A', 170), 20, 86);

        // Section: Other fields
        doc.setFont('helvetica', 'bold');
        doc.text('Aircraft', 20, 100);
        doc.setFont('helvetica', 'normal');
        doc.text((document.getElementById('pp-aircraft')?.value || '____'), 45, 100);
        doc.setFont('helvetica', 'bold');
        doc.text('Cruise Level', 120, 100);
        doc.setFont('helvetica', 'normal');
        doc.text((document.getElementById('pp-cruise')?.value || '____'), 160, 100);
